...
bool GameField::can_place_ship(Ship* ship, int x, int y, Orientation orientation) const
{
    ...
    for (int i = 0; i < length; ++i)
    {
        ...
        if (!is_correct_position(nx, ny) || field[ny][nx] != CellStatus::Empty || touch_other_ship(nx, ny))
        {
            return false;
        }
    }
...
bool GameField::touch_other_ship(int x, int y) const
{
    for (int dx = -1; dx <= 1; ++dx)
    {
        for (int dy = -1; dy <= 1; ++dy)
        {
            int adj_x = x + dx;
            int adj_y = y + dy;
            if (is_correct_position(adj_x, adj_y) && field[adj_y][adj_x] == CellStatus::Ship)
            {
                return true;
            }
        }
...
void GameField::attack(int x, int y, ShipManager& shipManager)
{
    ...
    else if (field[y][x] == CellStatus::Empty)
    {
